...
function StatusBadge({ status }) {
  const mapping = {
    ...
    passed: 'border-emerald-500 bg-emerald-500/10 text-emerald-200',
    ...
    <ul className="space-y-2">
      {entries.map((entry) => (
        ...
        >
          ...
          <span className="font-mono text-emerald-300">{entry.score.toLocaleString()}</span>
        </li>
      ))}
    </ul>
  );
...
export function GauntletPlay() {
  ...
  return (
    <section className="grid gap-8 lg:grid-cols-[2fr_1fr]">
      <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-2xl shadow-emerald-500/5 backdrop-blur">
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-3">
            <h2 className="text-2xl font-semibold text-white">Today&apos;s Gauntlet</h2>
            <p className="text-sm uppercase tracking-[0.3em] text-emerald-400">{todayId}</p>
          </div>
                    ...
                    status === 'passed'
                      ...
                      : status === 'failed'
                        ...
                        : status === 'skipped'
                          ...
                          : status === 'active'
                            ? 'border-emerald-400/60 bg-slate-900/60 shadow-emerald-500/20'
          ...
          <div className="rounded-2xl border border-white/5 bg-slate-950/60 p-6 shadow-inner shadow-black/40">
            {currentGame && ActiveGameComponent && !isComplete ? (
              <ActiveGameComponent
                ...
                onPass={() => gauntlet.recordPass(state.currentIndex)}
                onFail={() => gauntlet.recordFail(state.currentIndex)}
                onSkip={() => gauntlet.recordSkip(state.currentIndex)}
              />
            ) : (
              <div className="space-y-4 text-center">
                ...
                <div className="grid grid-cols-2 gap-3 text-left text-sm">
                  <div className="rounded-xl border border-white/5 bg-white/5 p-4">
                    <p className="text-xs uppercase tracking-[0.3em] text-emerald-400">Score</p>
                    <p className="text-2xl font-bold text-white">{summary.score.toLocaleString()}</p>
                  </div>
                  <div className="rounded-xl border border-white/5 bg-white/5 p-4">
                    <p className="text-xs uppercase tracking-[0.3em] text-emerald-400">Time</p>
                    <p className="text-2xl font-bold text-white">{formatDuration(summary.totalTime)}</p>
                  </div>
                  <div className="rounded-xl border border-white/5 bg-white/5 p-4">
                    <p className="text-xs uppercase tracking-[0.3em] text-emerald-400">Passes</p>
                    <p className="text-2xl font-bold text-white">{summary.passes}</p>
                  </div>
                  <div className="rounded-xl border border-white/5 bg-white/5 p-4">
                    <p className="text-xs uppercase tracking-[0.3em] text-emerald-400">Skips / Fails</p>
      ...
      <aside className="space-y-6">
        <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-2xl shadow-emerald-500/5 backdrop-blur">
          ...
          <div className="mt-4 space-y-6">
            <div>
              <h4 className="text-xs uppercase tracking-[0.3em] text-emerald-400">Daily</h4>
              <LeaderboardList entries={leaderboard.daily} />
            </div>
            <div>
              <h4 className="text-xs uppercase tracking-[0.3em] text-emerald-400">This Week</h4>
              <LeaderboardList entries={leaderboard.weekly} />
            </div>
            <div>
              <h4 className="text-xs uppercase tracking-[0.3em] text-emerald-400">All Time</h4>
