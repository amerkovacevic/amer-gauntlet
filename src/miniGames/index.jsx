...
export const miniGames = [
  ...
  buildMultipleChoiceGame('factor-finder', 'Factor Finder', (random) => {
    const factorA = Math.floor(random() * 7) + 2;
    const factorB = Math.floor(random() * 8) + 2;
    const base = factorA * factorB;
    const answer = random() > 0.5 ? factorA : factorB;
    const options = new Set([answer]);
    while (options.size < 4) {
      const candidate = Math.floor(random() * base) + 2;
      if (base % candidate !== 0) {
        options.add(candidate);
      }
    }
    return {
      prompt: `Which number is a factor of ${base}?`,
  ...
  buildArithmeticInputGame('mental-ladder', 'Mental Ladder', 'Climb the ladder', (random) => {
    const start = Math.floor(random() * 30) + 10;
    const step = Math.floor(random() * 5) + 2;
    const climbs = Math.floor(random() * 3) + 3;
    const total = start + step * climbs;
    return {
      prompt: `Start at ${start} and add ${step} a total of ${climbs} times. Where do you land?`,
      answer: total,
      hint: 'Keep track of each jump forward.',
    };
  }),
  buildTrueFalseGame('even-square', 'Even Square', (random) => {
    const base = Math.floor(random() * 20) + 5;
    const prompt = `The square of ${base} is even.`;
    return {
      prompt,
      answer: base % 2 === 0,
    };
  }),
  buildMultipleChoiceGame('angle-finder', 'Angle Finder', (random) => {
    let first = (Math.floor(random() * 6) + 2) * 10;
    let second = (Math.floor(random() * 6) + 2) * 10;
    if (first + second >= 160) {
      second = 150 - first;
    }
    ...
    const options = new Set([answer]);
    while (options.size < 4) {
      const candidate = answer + (Math.floor(random() * 5) - 2) * 5;
      if (candidate > 0 && candidate !== answer) {
        options.add(candidate);
      }
    }
    return {
      prompt: `A triangle has angles of ${first}° and ${second}°. What is the third angle?`,
      options: Array.from(options)
        .map(String)
        .sort(() => random() - 0.5),
      answer: String(answer),
  ...
  buildMultipleChoiceGame('unit-convert', 'Unit Convert', (random) => {
    const centimeters = (Math.floor(random() * 19) + 12) * 10;
    const answer = (centimeters / 100).toFixed(2);
    const options = new Set([answer]);
    while (options.size < 4) {
      const delta = (Math.floor(random() * 5) - 2) * 0.1;
      const candidate = (centimeters / 100 + delta).toFixed(2);
      options.add(candidate);
    }
    return {
      prompt: `Convert ${centimeters} centimeters to meters.`,
      options: Array.from(options).sort(() => random() - 0.5),
      answer,
    };
  }),
  buildOrderGame('descending-order', 'Descending Order', (random) => {
    const values = Array.from({ length: 4 }, () => Math.floor(random() * 90) + 10);
    const sequence = [...values].sort((a, b) => b - a);
    return {
      prompt: 'Select the numbers from largest to smallest.',
      options: values,
      sequence,
    };
  }),
  buildMultipleChoiceGame('mean-mania', 'Mean Mania', (random) => {
    const start = Math.floor(random() * 15) + 5;
    const step = Math.floor(random() * 5) + 1;
    const numbers = [start, start + step, start + step * 2];
    const answer = start + step;
    const options = new Set([answer]);
    while (options.size < 4) {
      const candidate = answer + Math.floor(random() * 5) - 2;
      options.add(candidate);
    }
    return {
  ...
  buildTrueFalseGame('prime-plus', 'Prime Plus', (random) => {
    const primes = [5, 7, 11, 13, 17, 19, 23, 29];
    const base = primes[Math.floor(random() * primes.length)];
    ...
    return {
      prompt: `${base} + 2 is prime.`,
      answer: isPrime(candidate),
    };
  }),
  buildMultipleChoiceGame('nearest-square', 'Nearest Square', (random) => {
    const target = Math.floor(random() * 150) + 50;
    const root = Math.sqrt(target);
    const lower = Math.floor(root);
    const upper = lower + 1;
    const candidates = [lower * lower, upper * upper];
    if (lower - 1 > 0) {
      candidates.push((lower - 1) * (lower - 1));
    }
    const answer = candidates.reduce((best, value) => {
      const bestDiff = Math.abs(best - target);
      const diff = Math.abs(value - target);
      if (diff < bestDiff) return value;
      if (diff === bestDiff) return Math.min(best, value);
      return best;
    }, candidates[0]);
    const options = new Set([answer]);
    while (options.size < 4) {
      const n = Math.floor(random() * 12) + 2;
      options.add(n * n);
    }
    return {
      prompt: `Which perfect square is closest to ${target}?`,
      options: Array.from(options)
        .map(String)
        .sort(() => random() - 0.5),
      answer: String(answer),
  ...
  buildMultipleChoiceGame('remainder-quest', 'Remainder Quest', (random) => {
    const divisor = Math.floor(random() * 6) + 4;
    const quotient = Math.floor(random() * 9) + 2;
    const remainder = Math.floor(random() * divisor);
    const dividend = divisor * quotient + remainder;
    const options = new Set([remainder]);
    while (options.size < 4) {
      const candidate = Math.floor(random() * divisor);
      options.add(candidate);
    }
    return {
